...
function benchmarkUtilityOperations(trie: SeaDix, dataset: string[], datasetName: string): Benchmark {
	...
	// getHeightStats calls
	const heightStatsTime = benchmark.time(() => {
		trie.getHeightStats();
	}, 100);
	benchmark.addResult("getHeightStats() (100x)", heightStatsTime, 100);

	// getMemoryStats calls
	const memoryStatsTime = benchmark.time(() => {
		trie.getMemoryStats();
	}, 100);
	benchmark.addResult("getMemoryStats() (100x)", memoryStatsTime, 100);

	// getWordMetrics calls
	const wordMetricsTime = benchmark.time(() => {
		trie.getWordMetrics();
	}, 100);
	benchmark.addResult("getWordMetrics() (100x)", wordMetricsTime, 100);

	// patternSearch calls (simple pattern)
	const patternSearchTime = benchmark.time(() => {
		trie.patternSearch("*a*");
	}, 100);
	benchmark.addResult("patternSearch('*a*') (100x)", patternSearchTime, 100);
